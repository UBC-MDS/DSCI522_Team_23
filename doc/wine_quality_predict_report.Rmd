...
Our final regression model- k-nearest neighbors (k-NN) regressor performs fairly well on an unseen test data set, with a RMSE score of 0.7171. 
...
We selected k-NN regressor as our final model after making comparison with the baseline model, dummy regressor and the ridge regressor based on the mean cross-validation RMSE score with hyperparameter tuning. We then used the best tuned hyperparameter, k= 16 for k-NN regressor and alpha=10 for ridge regressor to obtain and overall RME score for out training set. We can see that it is obvious the K-nn regressor has a lower RMSE score compared to other models. 

```{r}
kable(read.csv(file = '../results/model_comparison.csv'), caption = 'Table 2: RMSE Score for All Models Training Set')
...
```{r, fig.cap= "Figure 2. Number of Wines by Quality Class", out.width='100%'}
...
```{r}
kable(read.csv(file = '../results/knn_results.csv'), caption = 'Table 3: Cross-validation RMSE Score for kNN')
```

```{r}
kable(read.csv(file = '../results/ridge_results.csv'), caption = 'Table 4: Cross-validation RMSE Score for Ridge Regressor')
```
...
```{r}
kable(read.csv(file = '../results/dummy_results.csv'), caption = 'Table 5: Cross-validation RMSE Score for Dummy Regressor')
```

We also compare the actual vs prediction score values for our validation set on the kNN and Ridge models. From here, we can see that the predicted quality falls within the range of 4 to 8, which are reasonable values. One of our concerns when performing regression on our data was the predictions fall out our quality score range, which is between 0-10. 

```{r, fig.cap= "Figure 3. Ridge Predictions vs Actual", out.width='100%'}
knitr::include_graphics("../results/ridge_prediction.png")
```

```{r, fig.cap= "Figure 4. KNN Predictions vs Actual", out.width='100%'}
...
```{r}
kable(read.csv(file = '../results/test_set_result.csv'), caption = 'Table 1: RMSE Score for Test Sset')
```
